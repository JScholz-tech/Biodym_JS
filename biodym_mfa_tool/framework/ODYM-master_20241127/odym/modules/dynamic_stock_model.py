                ...
                for m in range(
                    ...
                    if (
                        ...
                        if self.sf[m, m] != 0:  # Else, inflow is 0.
                            self.i[m] = (
                                (self.s[m] - self.s_c[m, :].sum()) / self.sf[m, m]
                ...
                if NegativeInflowCorrect is True:
                    for m in range(
                        ...
                        if InflowTest < 0:
                            ...
                            self.s_c[m::, 0:m] = (
                                self.s_c[m::, 0:m] * (1 - Delta_percent.copy())
                            )  # shrink future description of stock from previous age-cohorts by factor Delta_percent in current AND future years.
                        else:
                            if self.sf[m, m] != 0:  # Else, inflow is 0.
                                self.i[m] = (
                ...
                if NegativeInflowCorrect is True:
                    for m in range(
                        ...
                        if i0_test < 0:
                            ...
                            s_cg[m::, 0:m, :] = (
                                s_cg[m::, 0:m, :] * (1 - Delta_percent.copy())
                            )  # shrink future description of stock from previous age-cohorts by factor Delta_percent in current AND future years.
                            o_cg[m + 1 : :, :, :] = (
                                s_cg[m:-1, :, :] - s_cg[m + 1 : :, :, :]
