    ...
    def MassBalance(self, Element = None):
        ...
        for key in self.StockDict: # Add all stock changes to the mass balance
            if  self.StockDict[key].Type == 1:
                print(f"Prüfe Stock-Änderung: {key}")
                print(f"  Indices String: '{self.StockDict[key].Indices}'")
                if hasattr(self.StockDict[key].Values, 'shape'):
                    print(f"  Values Shape: {self.StockDict[key].Values.shape}")
                else:
                     print("  Values sind None oder kein numpy Array")
