...
def sankey_results(
    ...
    for i in range(len(MyYears)):
        ...
        for b in range(len(flows_P_Start)):
            loop_list.append(
                [
                    flows_Values[b][:][i][e]
                    for e in range(len(ModelClassification["Element"].Items))
                ]
            )
    ...
    def update(element, i=MyYears[0]):
        ...
        fig.data[0].link.value = [
            flows_Values_loop[i - (MyYears[0])][b][element_index]
            for b in range(len(flows_Values))
        ]
...
def bar_stocks_results(Dyn_MFA_System, MyYears, ModelClassification, colors_processes):
    ...
    fig.add_bar(
        x=stocks_process_Names, y=stocks_Values_loop[0][0], marker_color=colors_stocks
    )
    ...
    def update(element, i=MyYears[0]):
        element_index = elements.index(element)
        fig.data[0].y = [
            stocks_Values_loop[i - MyYears[0]][b][element_index]
            for b in range(len(stocks_Values))
        ]
