...
def export_xlsx(Dyn_MFA_System, MyYears, ModelClassification):
    ...
    for element in ModelClassification["Element"].Items:
        ...
        for flow in Dyn_MFA_System.FlowDict:
            flows_Values.append(
                Dyn_MFA_System.FlowDict[flow].Values[
                    :, ModelClassification["Element"].Items.index(element)
                ]
            )
        ...
        for stock in Dyn_MFA_System.StockDict:
            stock_Values.append(
                Dyn_MFA_System.StockDict[stock].Values[
                    :, ModelClassification["Element"].Items.index(element)
                ]
            )
...
def MC_export_xlsx(MC_FlowDict, MC_StockDict, MyYears, ModelClassification):
    ...
    for element in ModelClassification["Element"].Items:
        ...
        mean_flows = np.mean(
            flow_values, axis=1
        )  # Mean over first dimension, shape (n_flows, 15)
        std_flows = np.std(flow_values, axis=1)
        df_mean_flows = pd.DataFrame(
            mean_flows.T, columns=flow_labels
        )  # Transpose to match years as rows
