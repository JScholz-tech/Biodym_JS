...
biodym_addon_path = os.path.join(project_root_parent, 'framework', 'bioDYM_add-on', 'modules')
...
def main():
    ...
    if config.RUN_MONTE_CARLO:
        if df_mc_results is not None and not df_mc_results.empty:
            ...
            plotting.plot_mc_distribution(df_mc_results, 'final_C_stock_soil', unit='Mg C')
            
            uncertain_input_to_test = 'fomp_8_k1'
            if uncertain_input_to_test in df_mc_results.columns:
                plotting.plot_mc_sensitivity_scatter(df_mc_results, uncertain_input_to_test, 'final_C_stock_soil', unit='Mg C')
        else:
