...
input_data = pd.read_excel('data/01_input/250707_Template_CS1.xlsx', sheet_name=None, header=0, engine='openpyxl', na_values=['N.A.', 'NA', 'n/a'])
data_loader.validate_input_data(input_data)
print("✅ Data validation passed!")

# 3. Model setup
model_classification, index_table = system_setup.define_model_scope(
    config_obj.Start_Year, config_obj.End_Year, getattr(config_obj, 'Elements_comma-separated').split(',')
)
...
mfa_system_configured, all_excel_data = system_setup.define_flows_and_parameters(mfa_system_base, all_excel_data)
