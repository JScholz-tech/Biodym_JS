...
def create_golden_dataset():
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        ...
        pd.DataFrame(config_data).to_excel(
            writer, sheet_name="0_Configuration", index=False
        )
        ...
        pd.DataFrame(process_data).to_excel(
            writer, sheet_name="2_1_Definition_Processes", index=False
        )
        ...
        pd.DataFrame(flow_data).to_excel(
            writer, sheet_name="1_1_Definition_Flows", index=False
        )
        ...
        pd.DataFrame(flow_time_data).to_excel(
            writer, sheet_name="1_2_Data_Flows", index=False
        )
        ...
        pd.DataFrame(tc_data).to_excel(
            writer, sheet_name="2_3_Process_TCs", index=False
        )
        ...
        dsm_df = pd.DataFrame(
            {
                ...
                "Lifetime_StdDev": [
                    ...
                    0,
                    0,
                ],  # This column can remain, but is ignored for 'Fixed'
            }
        )
        ...
        pd.DataFrame(uncertainty_data).to_excel(
            writer, sheet_name="4_1_Uncertainty_Parameters", index=False
        )
...
def test_golden_dataset():
    ...
    try:
        ...
        mfa_system_configured, all_excel_data = (
        ...
        print("\n📊 VALIDATION RESULTS:")
        print(
            f"   Flows validated: {validation_results['flows_correct']}/{validation_results['flows_total']}"
        )
        print(
            f"   Stocks validated: {validation_results['stocks_correct']}/{validation_results['stocks_total']}"
        )
        print(
            f"   Mass balance: {'✅ PASS' if validation_results['mass_balance_correct'] else '❌ FAIL'}"
        )
        print(
            f"   Overall: {'✅ PASS' if validation_results['overall_pass'] else '❌ FAIL'}"
        )
...
def validate_results(mfa_system, expected_results, tolerance=1e-6):
        ...
        if flow_id in mfa_system.FlowDict:
            ...
            for dim_name, expected_dim_values in expected_values.items():
                ...
                if len(calculated_dim_values) == len(expected_dim_values):
                    max_diff = max(
                        abs(calc - exp)
                        for calc, exp in zip(calculated_dim_values, expected_dim_values)
                    )
                    if max_diff <= tolerance:
                        validation["flows_correct"] += 1
                    else:
                        print(
                            f"   ❌ Flow {flow_id} dimension {dim_name}: max difference = {max_diff}"
                        )
    ...
    for stock_id, expected_values in expected_results["stocks"].items():
        if stock_id in mfa_system.StockDict:
            ...
            if len(calculated_values) == len(expected_values):
                max_diff = max(
                    abs(calc - exp)
                    for calc, exp in zip(calculated_values, expected_values)
                )
                if max_diff <= tolerance:
    ...
    process_1_flows = [
        f for f in mfa_system.FlowDict.values() if f.P_End == 1 or f.P_Start == 1
    ]
    if process_1_flows:
    ...
    if "F_03_02" in expected_results["flows"]:
        ...
        for dim_name, exp_vals in expected_results["flows"]["F_03_02"].items():
            ...
            if max_diff <= tolerance:
                print(
                    f"   ✅ DSM outflow F_03_02 ({dim_name}) matches expected values."
                )
            else:
                print(
                    f"   ❌ DSM outflow F_03_02 ({dim_name}): max difference = {max_diff}"
                )
                validation["overall_pass"] = False
