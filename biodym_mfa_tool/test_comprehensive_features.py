...
def test_monte_carlo_visualization():
    ...
    try:
        plotting.plot_mc_sensitivity_scatter(mc_results, 'TC_1', 'final_C_stock_soil', unit='Mg C')
    ...
    try:
        plotting.plot_mc_confidence_intervals(mc_results, 'final_C_stock_soil', unit='Mg C')
...
def test_scenario_comparison():
    ...
    try:
        ...
        scenario_manager.save_scenario("extended", extended_config, "Extended time horizon")
        scenario_manager.save_scenario("high_efficiency", high_efficiency_config, "High efficiency scenario")
        ...
        differences = scenario_manager.get_scenario_differences(["baseline", "extended"])
        print(f"✅ Found {len([k for k, v in differences.items() if v['different']])} differences between baseline and extended")
        ...
        print(f"   Top scenario: {ranking['ranking'][0][0]} (score: {ranking['ranking'][0][1]:.3f})")
            ...
            try:
                ...
                if 'Configuration' in input_data:
                    try:
                        ...
                        print(f"   ✅ Configuration loaded from Excel")
                        print(f"      Start Year: {getattr(config_obj, 'Start_Year', 'N/A')}")
                        print(f"      End Year: {getattr(config_obj, 'End_Year', 'N/A')}")
                    except Exception as e:
    ...
    try:
        ...
        class AnalysisConfig:
            def __init__(self, config_obj):
                self.excel_file_path = getattr(config_obj, 'Input_File_Path', 'data/01_input/250707_Template_CS1.xlsx')
                self.output_path = getattr(config_obj, 'Output_File_Path', 'data/02_output/results.xlsx')
                self.start_year = getattr(config_obj, 'Start_Year', 2025)
                self.end_year = getattr(config_obj, 'End_Year', 2050)
                self.elements = getattr(config_obj, 'Elements_comma_separated', 'material,WC,DM,CC').split(',')
                self.run_monte_carlo = getattr(config_obj, 'Run_Monte_Carlo_Simulation', False)
                self.mc_iterations = getattr(config_obj, 'Monte_Carlo_Iterations', 100)
                self.RUN_DSM_CALCULATION = getattr(config_obj, 'Run_DSM_Calculation', True)
                self.RUN_FOMP_CALCULATION = getattr(config_obj, 'Run_FOMP_Calculation', True)
        ...
        model_classification, index_table = system_setup.define_model_scope(
            analysis_config.start_year, analysis_config.end_year, analysis_config.elements
        )
