...
def model_to_yaml(excel_data: dict, source_file: str = "") -> dict:
    ...
    if cfg_df is not None:
        ...
        _elem_num_to_name: dict[int, str] = {num: name_val for num, name_val in elem_pairs}
    ...
    if proc_df is not None and "ID" in proc_df.columns:
        ...
        if "Process_Name" in valid.columns:
            ...
            valid = valid[valid["Process_Name"].astype(str).str.strip().str.lower() != "nan"]
        for _, row in valid.iterrows():
            ...
            if "Stock_Configuration" in row and pd.notna(row.get("Stock_Configuration")):
    ...
    if flow_df is not None and "Flow_ID" in flow_df.columns:
        ...
        for _, row in valid_flows.iterrows():
            ...
            if "Flow_Output_Process_ID" in row and pd.notna(row.get("Flow_Output_Process_ID")):
    ...
    if fomp_df is not None and not fomp_df.empty:
        ...
        for _, row in fomp_df.iterrows():
            ...
            pname = None
            val = None
            if "FOMP_Parameter_ID" in fomp_df.columns and pd.notna(row.get("FOMP_Parameter_ID")):
                ...
                if raw_id.startswith("P") and "_" in raw_id:
                    try:
                        ...
                        pass
            elif "FOMP_Parameter_type" in fomp_df.columns and pd.notna(row.get("FOMP_Parameter_type")):
                pid_raw = row.get("Process_ID")
                pid = int(float(pid_raw)) if pid_raw is not None and not (isinstance(pid_raw, float) and pd.isna(pid_raw)) else None
    ...
    if dsm_df is not None and not dsm_df.empty:
        ...
        for pid, grp in dsm_df.groupby("_pid"):
            ...
            if "DSM_Parameter_type" in grp.columns and "DSM_Value" in grp.columns:
                ...
                if "DSM_material_count" in grp.columns and grp["DSM_material_count"].notna().any():
                    cat_nums = sorted(grp["DSM_material_count"].dropna().unique())
                    cat_groups = [(n, grp[grp["DSM_material_count"] == n]) for n in cat_nums]
                else:
                    cat_groups = [(1, grp)]
                for _cat_n, cat_grp in cat_groups:
                    ...
                    categories_out.append({
                        ...
                        "lifetime_type":  str(pm.get("DSM_Lifetime_Type", "Normal")).strip(),
                ...
                for _, r in grp.iterrows():
                    ...
                    categories_out.append({
                        ...
                        "lifetime_type":  str(r.get("Lifetime_Type", "Normal")).strip() if pd.notna(r.get("Lifetime_Type")) else "Normal",
    ...
    _active_elem_indices = list(range(2, len(elements) + 1))  # E1=material skipped (used for BOM)
    _tc_elem_indices = list(range(1, len(elements) + 1))      # includes E1=material (used for TCs)
    ...
    if proc_df is not None and "ID" in proc_df.columns:
        for _, row in proc_df.dropna(subset=["ID"]).iterrows():
            ...
            if "TC_Configuration" in row.index and pd.notna(row.get("TC_Configuration")):
    ...
    if bom_df is not None and "Process_ID" in bom_df.columns and elements:
        ...
        for pid_raw, grp in bom_clean.groupby(bom_clean["Process_ID"].apply(lambda x: int(float(x)))):
            pid = int(pid_raw)
            bom_flows = []
            for _, row in grp.iterrows():
                fid = str(row.get("Flow_ID", "")).strip()
                ...
                if flow_type == "target_Product":
                    for n in _active_elem_indices:
                        col = f"E{n}_TC_Value[%]"
                        if col in row.index and pd.notna(row[col]):
                            elem_name = elements[n - 1] if n - 1 < len(elements) else f"E{n}"
    ...
    if flow_df is not None and "Flow_ID" in flow_df.columns and elements:
        ...
        _old_frac_map = {"WC": "Flow_WC[%]", "DM": "Flow_DM[%]", "CC": "Flow_CC_DM[%]", "TC": "Flow_CC_DM[%]"}
    ...
    if flow_data_df is not None and "Flow_ID" in flow_data_df.columns and "Flow_Data_Year" in flow_data_df.columns:
        material_col = (
            "E1_value" if "E1_value" in flow_data_df.columns
            else "Flow_Material" if "Flow_Material" in flow_data_df.columns
            else None
        )
        if material_col:
            ...
            for fid, yv in sorted(flow_data_acc.items()):
                out["flow_data"].append({"flow_id": fid, "element": "material", "values": yv})
    ...
    if _mc_sheet in excel_data:
        ...
        _id_col = "MC_Parameter_ID" if "MC_Parameter_ID" in df_mc.columns else "Parameter_Name"
        ...
        for _, row in df_mc.iterrows():
            ...
            flow_group = str(flow_group_raw).strip() if pd.notna(flow_group_raw) and str(flow_group_raw).strip() else None
