...
def _(mo):
    ...
    mfa_state, set_mfa_state = mo.state(None)
    return file_upload, mfa_state, run_btn, set_mfa_state
...
@app.cell
def _(mfa_state, mo, plotting):
    if mfa_state() is None:
        t_sankey = mo.callout(mo.md("Run analysis to generate the Sankey diagram."), kind="info")
    else:
        _d = mfa_state()
        try:
            with mo.capture() as _cap:
                plotting.plot_interactive_sankey(_d["mfa_results"], _d["dsm_params"], _d["fomp_params"])
            t_sankey = _cap.as_html()
...
def _(mfa_state, mo, plotting):
    if mfa_state() is None:
        t_flows = mo.callout(mo.md("Run analysis to see flow dynamics."), kind="info")
    else:
        _d = mfa_state()
        try:
            with mo.capture() as _cap:
                plotting.plot_flow_dynamics(_d["mfa_results"])
                plotting.plot_process_dynamics(
                    _d["mfa_results"], _d["all_excel_data"]["2_1_Definition_Processes"]
                )
            t_flows = _cap.as_html()
        except Exception as _e:
...
@app.cell
def _(mfa_state, mo, plot_flow_composition):
    if mfa_state() is None:
        t_composition = mo.callout(mo.md("Run analysis to see composition plots."), kind="info")
    else:
        _d = mfa_state()
        try:
            with mo.capture() as _cap:
                plot_flow_composition(_d["mfa_results"])
            t_composition = _cap.as_html()
        except Exception as _e:
...
def _(mfa_state, mo, plotting):
    if mfa_state() is None:
        t_stocks = mo.callout(mo.md("Run analysis to see stock charts."), kind="info")
    else:
        _d = mfa_state()
        try:
            with mo.capture() as _cap:
                plotting.plot_stock_bar_chart(_d["mfa_results"], title="Stock Levels Over Time")
                plotting.plot_system_stock_composition(_d["mfa_results"])
            t_stocks = _cap.as_html()
        except Exception as _e:
...
def _(mfa_state, mo, plotting):
    if mfa_state() is None:
        t_models = mo.callout(mo.md("Run analysis to see process model results."), kind="info")
        has_models = False
    else:
        _d = mfa_state()
        _model_parts = []

        if _d["dsm_params"] and _d["dsm_details"]:
            try:
                with mo.capture() as _c:
                    plotting.plot_dsm_stock_details(_d["mfa_results"], _d["dsm_params"], _d["dsm_details"])
                    plotting.plot_dsm_process_dynamics(_d["mfa_results"], _d["dsm_params"], _d["dsm_details"])
        ...
        if _d["fomp_params"]:
            try:
                with mo.capture() as _c:
                    plotting.plot_fomp_stock_details(_d["mfa_results"], _d["fomp_params"])
                    if _d["fomp_details"]:
                        plotting.plot_fomp_pool_breakdown(_d["mfa_results"], _d["fomp_params"], _d["fomp_details"])
                    plotting.plot_fomp_dynamics(_d["mfa_results"], _d["fomp_params"])
                _model_parts.append(_c.as_html())
            except Exception as _e:
        ...
        if _d["lfg_params"]:
            try:
                with mo.capture() as _c:
                    plotting.plot_lfg_gas_production(_d["mfa_results"], _d["lfg_params"])
                    plotting.plot_lfg_fraction_breakdown(_d["mfa_results"], _d["lfg_params"])
                    plotting.plot_lfg_ipcc_vs_mfa_comparison(_d["mfa_results"], _d["lfg_params"])
                    plotting.plot_lfg_stock_details(_d["mfa_results"], _d["lfg_params"])
                _model_parts.append(_c.as_html())
            except Exception as _e:
...
def _(mfa_state, mo, plotting):
    if mfa_state() is None:
        t_scenarios = mo.callout(mo.md("Run analysis to see scenario comparisons."), kind="info")
    else:
        _d = mfa_state()
        if _d["has_scenarios"] and _d["all_scenario_results"]:
            try:
                with mo.capture() as _cap:
                    plotting.plot_multi_scenario_comparison(
                        _d["mfa_results"], _d["all_scenario_results"], _d["scenario_definitions"]
                    )
...
@app.cell
def _(calculate_stock_analysis, calculate_system_kpis, calculate_system_overview, mfa_state, mo):
    if mfa_state() is None:
        t_kpi = mo.callout(mo.md("Run analysis to calculate KPIs."), kind="info")
    else:
        _d = mfa_state()
        try:
            _kpi_df      = calculate_system_kpis(_d["mfa_results"], _d["process_logic_map"])
            _overview_df = calculate_system_overview(_d["mfa_results"], _d["process_logic_map"], _kpi_df)
            _stock_df    = calculate_stock_analysis(_d["mfa_results"])
            t_kpi = mo.vstack([
...
def _(mfa_state, mo, plotting):
    if mfa_state() is None:
        t_validation = mo.callout(mo.md("Run analysis to validate mass balance."), kind="info")
    else:
        _d = mfa_state()
        try:
            with mo.capture() as _cap:
                plotting.plot_total_mass_balance_error(_d["mfa_results"])
                plotting.plot_optimized_mass_balance_error(_d["mfa_results"])
            t_validation = _cap.as_html()
        except Exception as _e:
...
def _(
    ...
    _d_state = mfa_state()
    if _d_state is not None and _d_state.get("has_scenarios"):
        _tab_dict["📊 Scenarios"] = t_scenarios
    _tab_dict["📋 KPI"]         = t_kpi
    _tab_dict["✅ Validation"]  = t_validation
    mo.ui.tabs(_tab_dict)
...
@app.cell
def _(mc_btn, mfa_state, mo, plotting, run_mc_simulation):
    if mfa_state() is None:
        _mc_output = mo.callout(
            mo.md("Run the main analysis first, then use **🎲 Run Monte Carlo** for uncertainty analysis."),
            kind="info",
        )
    elif not mc_btn.value:
        _mc_output = mo.vstack([
            mo.md("## Monte Carlo Uncertainty Analysis"),
            mo.hstack([mc_btn], justify="start"),
        ])
    else:
        _d = mfa_state()
        try:
            _mc_results = run_mc_simulation(
                _d["mfa_configured"], _d["input_data"], _d["dsm_params"], _d["fomp_params"], _d["cfg"],
                process_logic_map=_d["process_logic_map"], flow_tc_map=_d["flow_tc_map"],
            )
        except Exception as _exc:
            mo.stop(True, mo.callout(mo.md(f"**MC failed:** {_exc}"), kind="danger"))

        if _mc_results is None or (hasattr(_mc_results, "empty") and _mc_results.empty):
            mo.stop(
                True,
                mo.callout(mo.md("⚠️  MC returned no results. Check MC_ITERATIONS in configuration."), kind="warn"),
            )

        _parts = [mo.md("## Monte Carlo Results"), mo.hstack([mc_btn], justify="start")]

        for _fn, _label in [
            (lambda: plotting.plot_interactive_mc_multiple_histograms(_mc_results, _d["mfa_results"]), "Histogram"),
            (lambda: plotting.plot_interactive_tornado(_mc_results), "Tornado"),
            (lambda: plotting.plot_interactive_mc_paths(_mc_results, _d["mfa_results"]), "Paths"),
            (lambda: plotting.plot_interactive_mc_stock_comparison(_mc_results, _d["mfa_results"]), "Comparison"),
        ]:
            try:
                with mo.capture() as _c:
                    _fn()
                _parts.append(_c.as_html())
            except Exception as _e:
                _parts.append(mo.callout(mo.md(f"{_label} failed: {_e}"), kind="warn"))

        _mc_output = mo.vstack(_parts)
    _mc_output
