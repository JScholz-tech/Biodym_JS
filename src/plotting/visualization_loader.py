...
def load_part6_visualization_sheets(excel_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if actual_process_sheet:
            process_colors_df = pd.read_excel(excel_file_path, sheet_name=actual_process_sheet, dtype=str)
            process_colors_df = _convert_df_decimal_columns(process_colors_df, ['X_Position', 'Y_Position', 'X_Position_Material', 'Y_Position_Material', 'X_Position_WC', 'Y_Position_WC', 'X_Position_DM', 'Y_Position_DM', 'X_Position_CC', 'Y_Position_CC'])
            key_col = 'Process_ID' if 'Process_ID' in process_colors_df.columns else 'ID'
            if key_col in process_colors_df.columns:
                ...
                config['process_colors'] = process_colors_df.set_index('_NORM_KEY').to_dict('index')
        ...
        if actual_flow_sheet:
            flow_colors_df = pd.read_excel(excel_file_path, sheet_name=actual_flow_sheet)
            ...
            if key_col in flow_colors_df.columns:
                ...
                flow_colors_df = flow_colors_df.drop_duplicates(subset=[key_col], keep='first')
                config['flow_colors'] = flow_colors_df.set_index(key_col).to_dict('index')
        ...
        if actual_layout_sheet:
            layout_df = pd.read_excel(excel_file_path, sheet_name=actual_layout_sheet)
...
def load_enhanced_visualization_config(excel_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if sheet_name:
            process_df = pd.read_excel(excel_file_path, sheet_name=sheet_name)
        ...
        if sheet_name:
            flow_df = pd.read_excel(excel_file_path, sheet_name=sheet_name)
        ...
        if sheet_name:
            element_df = pd.read_excel(excel_file_path, sheet_name=sheet_name)
...
def load_color_palette_config(excel_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if sheet_name:
            color_df = pd.read_excel(excel_file_path, sheet_name=sheet_name)
        ...
        if sheet_name:
            process_type_df = pd.read_excel(excel_file_path, sheet_name=sheet_name)
...
def get_default_visualization_config() -> Dict[str, Any]:
    ...
    return {
        'processes': {}, 'flows': {},
        'layout': {
            'Default_Layout_Type': 'Circular', 'Circular_Center_X': '0.5',
            'Circular_Center_Y': '0.5', 'Circular_Radius': '0.3',
            'Node_Spacing': '0.1', 'Flow_Curvature': '0.5',
            'Show_Flow_Labels': 'True', 'Show_Node_Labels': 'True',
            'Background_Color': '#FFFFFF', 'Grid_Color': '#E0E0E0'
        },
        'elements': {
        ...
        'advanced': {
            'Enable_Animation': 'True', 'Animation_Duration': '1000',
            'Enable_Zoom': 'True', 'Enable_Selection': 'True',
            'Export_Resolution': 'High', 'Export_Format': 'PNG'
        }
    }
