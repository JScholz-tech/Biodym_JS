...
Examples:
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="01_data/02_output/results.xlsx",
        help="Path for the output Excel file (default: 01_data/02_output/results.xlsx)",
    )

    parser.add_argument("--start-year", type=int, help="Start year for the analysis")

    parser.add_argument("--end-year", type=int, help="End year for the analysis")

    parser.add_argument("--elements", nargs="+", help="Elements to track")
    ...
    parser.add_argument(
        "--iterations", type=int, help="Number of Monte Carlo iterations"
    )

    parser.add_argument(
        "--no-export", action="store_true", help="Skip exporting results to Excel file"
    )
...
def run_mfa_analysis(args):
    ...
    try:
        ...
        print(
            f"   Setup complete: {len(mfa_system_configured.ProcessList)} processes, "
        ...
        if args.monte_carlo:
            print(
                f"   Running Monte Carlo simulation ({args.iterations} iterations)..."
            )

            mc_run_results = []
            iterator = (
                tqdm(range(args.iterations), desc="MC Progress")
                if not args.verbose
        ...
        if args.summary:
            ...
            if args.monte_carlo and df_mc_results is not None:
                if "final_C_stock_soil" in df_mc_results.columns:
                    ...
                    print(
                        f"     5th percentile: {final_c_stock.quantile(0.05):.2f} Mg C"
                    )
                    print(
                        f"     95th percentile: {final_c_stock.quantile(0.95):.2f} Mg C"
                    )
                ...
                if mfa_system_with_results is not None:
                    ...
                    for (
                        stock_name,
                        stock_obj,
                    ) in mfa_system_with_results.StockDict.items():
                        if stock_name.startswith("S_"):
                            final_values = stock_obj.Values[-1, :]
                            print(
                                f"     {stock_name}: {final_values[0]:.2f} Mg material"
                            )
        ...
        print("   - Use the Jupyter notebook for interactive visualizations")
        print("   - Modify parameters and re-run for different scenarios")
