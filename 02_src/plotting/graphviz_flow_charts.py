...
def plot_graphviz_flow_chart_sankey_style(
    processes_df,
    flows_df,
    ...
    try:
        ...
        complete_processes = complete_processes[
            complete_processes["Process_Name"].str.strip() != ""
        ]
        print(f"   - Complete processes: {len(complete_processes)}/{len(processes_df)}")

        # Filter flows with complete data
        complete_flows = flows_df.dropna(
            subset=[
                "Flow_ID",
                "Flow_Name",
        ...
        connected_processes = complete_processes[
            complete_processes["ID"].isin(connected_process_ids)
        ]
        ...
        if len(connected_processes) > max_processes:
            print(
                f"   ⚠️ Too many processes ({len(connected_processes)} > {max_processes})"
            )
            print("   📊 Selecting most connected processes...")
            ...
            for _, flow in complete_flows.iterrows():
                ...
                process_connections[process_o] = (
                    process_connections.get(process_o, 0) + 1
                )
                process_connections[process_i] = (
                    process_connections.get(process_i, 0) + 1
                )

            # Select most connected processes
            sorted_processes = sorted(
                process_connections.items(), key=lambda x: x[1], reverse=True
            )
            selected_process_ids = [pid for pid, _ in sorted_processes[:max_processes]]
            connected_processes = connected_processes[
                connected_processes["ID"].isin(selected_process_ids)
            ]
        ...
        if len(filtered_flows) > max_flows:
            ...
            print("   📊 Selecting most important flows...")
        ...
        print(
            f"   Final: {len(connected_processes)} processes, {len(filtered_flows)} flows"
        )
        ...
        dot.attr(
            ...
            penwidth=str(
                flow_thickness
            ),  # Thick edges create rectangular flow appearance
            splines="ortho",
            arrowhead="normal",
        )
