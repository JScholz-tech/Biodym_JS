...
class PublicationExporter:
    ...
    def get_export_path(
        self, plot_type, parameters=None, timestamp=None, use_subdir=True
    ):
    ...
    def export_plot(
        ...
        try:
            ...
            export_path = self.get_export_path(
                plot_type, parameters, use_subdir=use_subdir
            )
    ...
    def export_figure(
        ...
        try:
            if format == "png":
                ...
                fig.write_image(
                    filepath, width=settings["width"], height=settings["height"]
                )
            elif format == "svg":
                fig.write_image(
                    filepath, width=settings["width"], height=settings["height"]
    ...
    def batch_export(self, figures_dict, base_filename=None, formats=["png", "pdf"]):
        ...
        if base_filename is None:
            base_filename = (
                f"BioDYM_BatchExport_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
...
def create_publication_export_widget(fig, plot_type, element=None, process=None):
    ...
    format_dropdown = Dropdown(
        options=["png", "pdf", "svg", "html"], value="png", description="Format:"
    )
    ...
    export_button = Button(
        description="Export Figure", button_style="success", icon="download"
    )

    export_all_button = Button(
        description="Export All Formats", button_style="info", icon="download"
    )
...
def create_figure_summary(figures_dict):
    ...
    def on_export_all_click(b):
        base_filename = (
            f"BioDYM_CompleteAnalysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
...
def quick_export(fig, plot_type, element=None, process=None, format="png"):
    ...
    return exporter.export_figure(
        fig, plot_type, format=format, element=element, process=process
    )
