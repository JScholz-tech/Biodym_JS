...
def _calculate_outflow_from_inflows(total_inflow_values, params, time_vector):
    ...
    for i in range(len(inflow_split)):
        ...
        lifetime_type = lifetime_type[i] if isinstance(lifetime_type, list) else lifetime_type
        ...
        stock_from_inflows_by_cat.append(s_c.sum(axis=1) if s_c is not None else np.zeros(num_years))
        outflow_from_inflows_by_cat.append(o_c.sum(axis=1) if o_c is not None else np.zeros(num_years))

    return stock_from_inflows_by_cat, outflow_from_inflows_by_cat

def _calculate_outflow_from_initial_stock(initial_stock_vector, mean_lifetimes, num_years, num_elements):
...
def _distribute_and_assign_outflows(mfa_system, process_id, outflow_flows, outflow_from_inflows_by_cat, outflow_from_initial_stock_ts, params, total_inflow_values):
    ...
    final_outflows = [np.zeros(len(mfa_system.IndexTable.Classification["Time"].Items)) for _ in outflow_flows]
...
def calculate_dynamic_stock(mfa_system, dsm_params_config):
    ...
    if not outflow_flows:
        print(f"WARNING: No outflow defined for DSM process {process_id}. Cannot calculate.")
        return mfa_system, {}

    # --- Calculations ---
    stock_from_inflows_by_cat, outflow_from_inflows_by_cat = _calculate_outflow_from_inflows(total_inflow_values, params, time_vector)
    decaying_stock_ts, outflow_from_initial_stock_ts = _calculate_outflow_from_initial_stock(initial_stock_vector, params.get("lifetimes", {}).get("Mean", []), num_years, num_elements)

    # --- Assign values and prepare results ---
    _distribute_and_assign_outflows(mfa_system, process_id, outflow_flows, outflow_from_inflows_by_cat, outflow_from_initial_stock_ts, params, total_inflow_values)
